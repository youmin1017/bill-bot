...
func (_q *PaymentQuery) QueryLedger() *LedgerQuery {
...
func (_q *PaymentQuery) WithLedger(opts ...func(*LedgerQuery)) *PaymentQuery {
	query := (&LedgerClient{config: _q.config}).Query()
...
//		GroupBy(payment.FieldFromUserID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Payment.Query().
//		Select(payment.FieldFromUserID).
//		Scan(ctx, &v)
...
func (_q *PaymentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Payment, error) {
	...
	if query := _q.withLedger; query != nil {
		if err := _q.loadLedger(ctx, query, nodes, nil,
			func(n *Payment, e *Ledger) { n.Edges.Ledger = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PaymentQuery) loadLedger(ctx context.Context, query *LedgerQuery, nodes []*Payment, init func(*Payment), assign func(*Payment, *Ledger)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Payment)
	for i := range nodes {
		if nodes[i].ledger_payments == nil {
			continue
		}
		fk := *nodes[i].ledger_payments
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "ledger_payments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
